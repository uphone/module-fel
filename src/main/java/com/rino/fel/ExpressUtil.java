...
public final class ExpressUtil {
    ...
    // 简单格式化,不需要engine
    public static String simpleFormat(String el, Map context) {
        Pattern regex = Pattern.compile("\\$\\{(.*?)}");
        Matcher matcher = regex.matcher(el);
        StringBuffer ret = new StringBuffer();
        while (matcher.find()) {
            String express = matcher.group();
            String key = express.substring(0, express.length() - 1).substring(2);
    ...
    private static void setCp(Object cp, FelEngine engine) {
        ...
        final FelContext ctx = context;
        if (cp instanceof Map) {
            ((Map) cp).forEach((k, v) -> ctx.set((String) k, v));
        } else {
